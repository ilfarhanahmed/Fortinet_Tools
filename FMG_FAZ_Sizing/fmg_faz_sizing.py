...
FMG_DEVICE_TIER_TOLERANCE_PERCENT = 60
FAZ_LOGRATE_TIER_TOLERANCE_PERCENT = 60
...
def get_required_fmg_tier(devices):
    """
    Select FMG sizing tier using tolerance only against the
    immediate previous tier.

    Example:
    - 1491 is above 1200.
    - Strict tier would be 4000.
    - Previous tier is 1200.
    - 1200 + 60% = 1920.
    - Therefore, keep the 1200-device tier.
    """

    previous_row = None

    for row in FMG_SIZING:
        tier_devices = row[0]

        if devices <= tier_devices:
            if previous_row:
                previous_devices = previous_row[0]
                previous_allowed_devices = previous_devices * (
                    1 + FMG_DEVICE_TIER_TOLERANCE_PERCENT / 100
                )

                if devices <= previous_allowed_devices:
                    return previous_row, True, previous_allowed_devices

            return row, False, tier_devices

        previous_row = row

    return FMG_SIZING[-1], False, FMG_SIZING[-1][0]


def get_required_faz_tier(effective_rate):
    """
    Select FAZ sizing tier using tolerance only against the
    immediate previous tier.

    Example:
    - 10673 is above 10000.
    - Strict tier would be 20000.
    - Previous tier is 10000.
    - 10000 + 60% = 16000.
    - Therefore, keep the 10000 logs/sec tier.

    License checks still use the exact effective rate.
    """

    previous_row = None

    for row in FAZ_SIZING:
        tier_rate = row[0]

        if effective_rate <= tier_rate:
            if previous_row:
                previous_rate = previous_row[0]
                previous_allowed_rate = previous_rate * (
                    1 + FAZ_LOGRATE_TIER_TOLERANCE_PERCENT / 100
                )

                if effective_rate <= previous_allowed_rate:
...
def check_fmg(text, is_vm, platform_check):
    ...
    tier, tier_tolerance_used, tier_capacity = get_required_fmg_tier(
        devices
    )
    ...
    if tier_tolerance_used:
        info(
            f"Managed devices are within "
            f"{FMG_DEVICE_TIER_TOLERANCE_PERCENT}% tolerance "
            f"of {max_devices} device tier"
        )
    ...
    usage = (devices / tier_capacity) * 100
    ...
    if tier_tolerance_used:
        info(
            ...
            f"({devices}) divided by tolerance-adjusted tier capacity "
            f"({tier_capacity:.0f})"
        )
    else:
        info(
...
def check_faz(text, is_vm):
    ...
    tier, tier_tolerance_used = get_required_faz_tier(
        effective_rate
    )
    ...
    if tier_tolerance_used:
        info(
            f"Effective rate is within "
            f"{FAZ_LOGRATE_TIER_TOLERANCE_PERCENT}% tolerance "
            f"of {req_rate} logs/sec tier"
        )
